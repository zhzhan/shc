...
trait Sedes[T] {
  ...
  def deserialize(bytes: Array[Byte], start: Int, end: Int): T
}

class DoubleSedes extends Sedes[Double] {
  def serialize(value: Double): Array[Byte] = ???
  override def deserialize(bytes: Array[Byte], start: Int, end: Int): Double = {
